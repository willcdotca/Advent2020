...
jukfrv
kvfujr`.split(/\n\n/g);
const person = a => a.replace(/\n/g, '').split('');
const firstAnswer = groups.reduce((responses, response) =>
    new Set(person(response)).size + responses, 0);

const secondAnswer = groups.reduce((responses, response) => {
  let peopleCount = response.split('\n').length;
  let allResponses = response.replace('\n', '');

  return responses + Array.from(new Set(person(response)))
    .filter(e =>
      allResponses.split(e).length - 1 === peopleCount).length;
}, 0);

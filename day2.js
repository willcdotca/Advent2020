...
const findValid = (data,valid) => {
  valid = valid || 0
  let line = data.pop()
  let pass = line[1]
  let rules = line[0].split(' ')
  let range = rules[0].split('-')
  let count = pass.split(rules[1]).length-1
  if(count >= range[0] && count <= range[1]) valid++
  if(data.length) return findValid(data,valid)
  console.log(valid)
}

findValid(raw.split(/\n/).map(a=>a.split(':')))

const findPlaces = (data,valid) => {
  valid = valid || 0
  let line = data.pop()
  let pass = line[1]
  let rules = line[0].split(' ')
  let range = rules[0].split('-')
  let first = pass[range[0]] === rules[1]
  let second = pass[range[1]] === rules[1]
  if((first || second) && !(first && second)) valid++
  if(data.length) return findPlaces(data,valid)
  console.log(valid)
}

findPlaces(raw.split(/\n/).map(a=>a.split(':')))
